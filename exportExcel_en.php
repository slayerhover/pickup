...
$cellname  = array(
		array('id','ID'),
		array('catanum','catanum'),
		array('name','name'),
		array('size','size'),
		array('price','price'),
		array('status','status'),		
		array('target','target'),
		array('pathway','pathway'),
		array('information','inormation'),
		array('thumb','thumb'),
		array('cas', 'CAS'),
		array('mw', 'MW'),
...
$dataset	= $_DB->getAll("SELECT * from products_en");        
$rows		= [];
foreach($dataset as $k=>$v){
	if(empty($v['priceset'])){
		...
		foreach($priceset as $k1=>$v1){
			...
			if($v1['size']=='10mM/1mL')	continue;
			if($v1['price']<50)	continue;
...
$outputFileName = "goods" . date('YmdHis') . "_en.xls"; //使用Excel2007导出.xlsx文件
